...
export function gerarRelatorio(inicio, fim, setorFiltro = null) {
  ...
  // ---- 4. Intervenções e abordagens do concierge ----
  const intervencoes = db.prepare(`
    SELECT i.*, q.paciente, q.setor AS setor_clinico
    FROM intervencoes i JOIN quartos q ON q.id = i.quarto_id
    WHERE i.em >= @ini AND i.em < @fim
    ORDER BY i.em DESC
  `).all(argsPeriodo);
  const intervPorAutor = {};
  for (const i of intervencoes) intervPorAutor[i.autor] = (intervPorAutor[i.autor] || 0) + 1;
  // separa: abordagem espontânea (sem avaliação vinculada) vs. resposta a nota baixa
  const espontaneas = intervencoes.filter((i) => !i.avaliacao_id).length;
  const respostaNota = intervencoes.length - espontaneas;
  ...
  return {
    ...
    intervencoes: {
      ...
      espontaneas,
      respostaNota,
      porAutor: Object.entries(intervPorAutor).map(([autor, n]) => ({ autor, total: n })),
      lista: intervencoes.map((i) => ({
        id: i.id, quarto_id: i.quarto_id, paciente: i.paciente || null,
        setor: i.setor_clinico, autor: i.autor, acao: i.acao,
        em: i.em, tipo: i.avaliacao_id ? "resposta_nota" : "espontanea",
      })),
    },
    leitosCriticos,
  };
